...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    ...
    const checks = sites.map(site => checkSite(site, env));
    await Promise.all(checks);
  }
};

async function checkSite(site: SiteCheck, env: Env): Promise<void> {
...
async function getHistory(env: Env, siteName: string | null): Promise<Response> {
  if (!siteName) {
